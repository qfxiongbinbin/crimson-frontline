...
一款可直接在浏览器运行的**原创**即时战略游戏（RTS）。基地建设、资源采集、部队生产、分批攻防——完整复刻经典 RTS 的核心循环。所有名称、阵营、单位、美术、音效均为原创：视觉素材全部由代码程序化绘制（Phaser Graphics 生成贴图），音效由 WebAudio 振荡器实时合成，不含任何第三方游戏资产。
...
`npm run smoke` 会用本机 Chrome 真实加载游戏，模拟鼠标键盘完成：部署指挥中心 → 建电站/精炼厂/兵营 → 采矿增收 → 点选与右键移动 → 生产步兵 → 自动交火 → 迷雾遮蔽 → AI 扩张 → 胜负判定，共 14 项断言，并输出截图到 `scripts/shots/`。
