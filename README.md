...
阻挡型工事会参与单位寻路；弹坑、警戒灯和路面允许穿行。工事本身不能延伸基地建造范围，避免通过廉价设施无限向地图外接龙。
...
`npm run smoke` 会用本机 Chrome 真实加载游戏，模拟鼠标键盘完成：部署指挥中心 → 建电站/精炼厂/兵营 → 采矿增收 → 点选与右键移动 → 生产步兵 → 工事放置与阻挡 → 自动交火 → 迷雾遮蔽 → AI 扩张 → 胜负判定，共 27 项断言，并输出截图到 `scripts/shots/`。
