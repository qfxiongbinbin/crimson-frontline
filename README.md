...
| 维修工厂 | 自动维修半径内的受伤作战单位；需要战车工厂与稳定供电 |
| 防御塔 | 自动攻击射程内敌人，电力不足时瘫痪 |
| 光棱塔 | 高耗电远程防御建筑，锁定目标后蓄能并发射高伤害棱镜光束 |

![维修工厂透明素材](src/assets/generated/buildings/repairFactory-neutral.png)

![光棱塔透明素材](src/assets/generated/buildings/prismTower-neutral.png)

建造有**固定范围限制**：每辆基地工程车部署后，会以新的指挥中心为核心生成一块独立的 8 格建造圈；电站、兵营、城墙等设施都不能继续向外延伸边界。想在远处开分基地，必须生产基地工程车、开到目标位置后再次部署。放置建筑时会显示所有指挥中心的半透明绿色边界；失去一个分基地不会立刻战败，只有全部指挥中心被摧毁才失败。科技建筑仍有前置要求（如战车工厂需要先建精炼厂）。
...
| 工事 | 行为 |
| --- | --- |
| 工兵路面 | 铺设 2×2 地面，不阻挡单位 |
| 岩石路障 | 高耐久的单格阻挡物 |
| 人工矿脉 | 部署 2×2 可采集矿区，共 1200 资源 |
| 沙袋墙 | 廉价的 2×1 防线 |
| 反坦克拒马 | 单格钢制障碍物 |
| 爆破弹坑 | 可穿行的低矮伪装工事 |
| 装甲残骸 | 利用废弃车辆形成掩体 |
...
阻挡型工事会参与单位寻路；弹坑、警戒灯和路面允许穿行。所有建筑与工事都不能延伸指挥中心划定的固定建造范围。

### 经济与电力

- 采矿车全自动运行：寻矿 → 开采 → 返回精炼厂 → 变现入账；矿脉会枯竭。
- 建筑耗电、电站供电；**电力不足时生产速度减半、防御塔、光棱塔与维修工厂停摆**，顶部电力指示会变红闪烁。
...
- 战车与建筑使用斜俯视透明素材，并叠加方向性投影、前后遮挡排序和移动悬挂起伏，形成不改变网格玩法的伪 3D 战场。
...
`npm run smoke` 会用本机 Chrome 真实加载游戏，模拟鼠标键盘完成：部署指挥中心 → 建电站/精炼厂/兵营 → 采矿增收 → 点选与右键移动 → 生产步兵 → 维修工厂自动回血 → 光棱塔蓄能攻击 → 工事放置与阻挡 → 基地车异地扩张 → 自动交火 → 迷雾遮蔽 → AI 扩张 → 胜负判定，共 34 项断言，并输出截图到 `scripts/shots/`。
