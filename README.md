...
- 建筑耗电、电站供电；**电力不足时生产速度减半、防御塔与维修工厂停摆**，顶部电力指示会变红闪烁。
...
`npm run smoke` 会用本机 Chrome 真实加载游戏，模拟鼠标键盘完成：部署指挥中心 → 建电站/精炼厂/兵营 → 采矿增收 → 点选与右键移动 → 生产步兵 → 维修工厂自动回血 → 工事放置与阻挡 → 基地车异地扩张 → 自动交火 → 迷雾遮蔽 → AI 扩张 → 胜负判定，共 32 项断言，并输出截图到 `scripts/shots/`。
